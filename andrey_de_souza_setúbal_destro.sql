...
-- Nome do aluno: Andrey de Souza Setúbal Destro
...
SELECT 
    NumeroPedido AS 'Número do Pedido', 
    COUNT(DISTINCT CodigoProduto) AS 'Total de Produtos Diferentes', 
...
SELECT 
    ...
    NomeVendedor AS 'Nome', 
    Salario AS 'Salário', 
    (SELECT Media FROM MediaSalarial) AS 'Média Salarial'
FROM 
    VENDEDORES
WHERE 
    Salario > (SELECT Media FROM MediaSalarial);
...
SELECT 
    ...
    pr.ValorUnitario AS 'Valor Unitário',
    ip.Quantidade AS 'Quantidade Comprada',
    (ip.Quantidade * pr.ValorUnitario) AS 'Valor Total',
    pd.PrazoEntrega AS 'Prazo de Entrega',
...
JOIN 
    ITEM_DE_PEDIDO ip ON pd.NumeroPedido = ip.NumeroPedido
JOIN 
    PRODUTOS pr ON ip.CodigoProduto = pr.CodigoProduto
JOIN 
    VENDEDORES v ON pd.CodigoVendedor = v.CodigoVendedor
ORDER BY 
    c.NomeCliente ASC, 
    pr.DescricaoProduto ASC, 
    ip.Quantidade DESC, 
    pd.PrazoEntrega ASC, 
    v.NomeVendedor ASC;
